...
public class SCDCDatabaseHelper extends SQLiteOpenHelper {
  ...
  public static final Table DATA_TABLE = new Table("data",
    Arrays.asList(new Column(COLUMN_NAME, "TEXT"),
                    // ACTION from data broadcast
                  new Column(COLUMN_TIMESTAMP, "FLOAT"),
                    // TIMESTAMP in data broadcast
                  new Column(COLUMN_VALUE, "TEXT")
    )); // JSON representing
  ...
  public static final String COLUMN_LABEL_DUE_DATE = "due_date";
  public static final Table LABEL_TABLE = new Table("label",
    Arrays.asList(new Column(COLUMN_LABEL_ID, "INT"), // label ID
                  ...
                  new Column(COLUMN_LABEL_IS_LOGGED, "INT"),
                    // is label being logged?
                  new Column(COLUMN_LABEL_START_LOGGING_TIME, "FLOAT"),
                    // TIMESTAMP when the logging started
                  new Column(COLUMN_LABEL_HAS_DUE_DATE, "INT"),
                    // does label have due date?
                  new Column(COLUMN_LABEL_DUE_DATE, "FLOAT")
                    // TIMESTAMP for the due date of alarm
    ));

  // File info table constants
  ...
  public void onCreate(SQLiteDatabase db) {
    ...
    db.execSQL(LABEL_TABLE.getCreateTableSQL());
    ...
    db.execSQL(String.format(Locale.KOREA, "insert into %s (%s, %s, %s, " +
                    "%s) values ('%s', '%s', '%s', %f)",
  ...
  /**
   * @description Drop and re-Create 'data' table.
   * @param db
   */
